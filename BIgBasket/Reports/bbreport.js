...
const vendorName = $event.query["Vendor Name"];
const month = $event.query["Month"]
console.log(vendorName + " " + month)

let timezoneOffset = 5.5 * 60 * 60 * 1000
let tillTime = moment().startOf('day').valueOf() - timezoneOffset;
let fromTime = moment().date(1).startOf('day').valueOf() - timezoneOffset
if(month == "Last Month"){
    tillTime = fromTime
    fromTime = moment().subtract(1, 'month').date(1).startOf('day').valueOf() - timezoneOffset
}
